...
**Dikonfirmasi 4 laporan**: Neraca (Balance Sheet), Laba Rugi (Profit & Loss), Trial Balance (Neraca Saldo), Buku Besar per akun (General Ledger detail). Laporan per Cost Center (lihat [01](01-accounting-foundation.md) poin 4) tetap relevan sebagai turunan/filter dari laporan-laporan ini, bukan laporan terpisah. **Ditambah 2026-08-30**: Buku Bantu Piutang & Buku Bantu Hutang (subsidiary ledger, breakdown akun Piutang/Hutang Usaha per partner) — gap ditemukan user saat review; dan Laporan Perubahan Ekuitas (Statement of Changes in Equity, wajib per PSAK 1) — sempat kelewat dari konfirmasi awal. Detail lengkap semuanya di 09.

**Detail field-level lengkap (filter, kolom, rumus, aturan timing "Laba Tahun Berjalan") dipindah ke dokumen sendiri: [09-laporan-keuangan.md](09-laporan-keuangan.md)** (2026-08-30) — scope-nya besar (4 laporan), dokumen ini cuma nyimpan aturan timing "Laba Tahun Berjalan" di bawah (baris 66-68) yang jadi rujukan silang dari 09.
...
- **Kas Masuk & Kas Keluar** — butuh `line_ids` (1 header bisa pecah ke beberapa akun lawan sekaligus, bukan cuma 1 jumlah tunggal):
  - Tiap line: `account_id` (akun lawan — akun sumber dana/pendapatan lain-lain utk Kas Masuk, akun biaya/lawan utk Kas Keluar; **wajib bukan tipe akun "Kas Bank"** — transaksi kas-ke-kas harus lewat form Transfer, bukan lewat Kas Masuk/Keluar), keterangan (per baris), nominal.
...
- **FIFO** — butuh tabel layer terpisah `c18.stock.layer`: `product_id`, tanggal (urutan konsumsi), qty asal, **qty sisa** (berkurang tiap konsumsi), harga satuan (dari baris dokumen sumber), referensi sumber (Penerimaan Barang / Retur Customer yang menambah layer ini).
...
**Laporan (diriset 2026-08-26, scope Basic dikonfirmasi — [kledo.com](https://kledo.com/blog/kartu-stok-barang/), [jurnal.id](https://www.jurnal.id/id/blog/kartu-stok-barang-adalah/), [help.accurate.id](https://help.accurate.id/product/accurate-online/fitur-aol/persediaan/perintah-stok-opname/mengenal-fitur-perintah-stok-opname/)) — SELESAI DIIMPLEMENTASIKAN & DIVERIFIKASI 2026-08-30**:
- **Kartu Stok per produk** — mutasi kronologis (tanggal, jenis transaksi/referensi, qty masuk/keluar, saldo qty berjalan, harga pokok saat itu, nilai). **Layout dirinci (2026-08-26)**:
  - **Filter/header laporan**: `product_id` (wajib, 1 kartu = 1 produk), rentang tanggal. Filter `cost_center_id` **TIDAK diimplementasikan** (2026-08-30) — tabel audit mutasi stok (`stock.layer`/`.consumption`/`.movement`) tidak menyimpan cost center sama sekali, nambahnya butuh ubah skema 3 tabel + semua pemanggilnya, di luar scope saat ini. Dicatat sebagai keterbatasan, bukan situasi diam-diam berbeda dari dokumen.
  ...
  **Perubahan mesin costing yang dibutuhkan (2026-08-30, ditemukan saat implementasi laporan ini)**: `c18.stock.layer` cuma nyimpan state SAAT INI (qty sisa), bukan histori per-transaksi, jadi split-per-layer di atas **tidak bisa direkonstruksi retroaktif** dari data yang ada. Ditambah 2 model audit log baru: **`c18.stock.consumption`** (breakdown per-layer per transaksi keluar FIFO, 1 baris = 1 layer yang kena konsumsi) dan **`c18.stock.movement`** (audit trail generik utk costing **Average** — sebelumnya TIDAK ADA histori mutasi sama sekali utk Average, cuma 2 field running `qty_on_hand`/`avg_cost` yang di-mutate tanpa jejak. Average itu **default company**, bukan edge case, jadi ini krusial). `product.py` method `_stock_consume`/`_stock_consume_latest`/`_stock_receive` diubah nulis ke tabel-tabel ini, 5 tempat pemanggil (`sale_delivery.py`, `sale_invoice.py`, `stock_consume.py`, `stock_opname.py`, `purchase_return.py`) diupdate kirim `res_model`/`res_id`/`date`.

**Rename model `c18.account.stock.*` → `c18.stock.*`** (2026-08-30, susulan — konsistensi penamaan, disamakan dengan `purchase`/`sale`/`payroll`/`fixed.asset` yang juga generate jurnal tapi tidak pakai prefix "account"): `layer`, `opname`, `consume`, `consumption`, `movement`, plus `c18.stock.report` (laporan). **Catatan penting kalau rename model serupa dilakukan lagi di masa depan ke database yang SUDAH punya data**: ganti `_name` di Odoo **tidak** rename tabel SQL lama secara otomatis — dia bikin tabel baru kosong, tabel lama jadi yatim (data masih ada secara fisik tapi tidak lagi terhubung ke model manapun via ORM). `qty_on_hand`/`avg_cost` di `c18.product` sendiri aman (field itu tidak ikut berubah), tapi histori detail (isi tabel layer/consumption/movement lama) butuh migration script manual (`ALTER TABLE ... RENAME TO ...`) kalau tidak mau hilang. Aman dilakukan sesi ini tanpa migration script karena **belum ada database produksi sama sekali** — `test-01` (satu-satunya database yang pernah terpasang) selalu dibersihkan dari data demo, jadi tabel lama kosong & langsung dihapus manual, bukan di-migrate.
- **Saldo Persediaan** — ringkasan qty on hand + nilai per produk saat ini; total harus cocok ke saldo akun Persediaan di Neraca. Ini tujuan awal fitur poin F ("saldo persediaan dirinci per produk"). **Selesai diimplementasikan** sekaligus dengan Kartu Stok (`get_inventory_balance_data()`).
...
### F.3 Sistem Pencatatan Persediaan: Perpetual vs Periodik (2026-08-30)

1 field baru `inventory_system` (selection: Perpetual/Periodik) di Settings company, terpisah dari `costing_method` (poin F) — **default Perpetual** (beda dari `costing_method` yang default-nya FIFO, krn ini fitur baru dan Perpetual = perilaku existing sebelum poin ini ada, tidak boleh diam-diam ganti untuk company yang sudah jalan). Berlaku **global** sama seperti `costing_method`, dan sama-sama cuma boleh diubah setelah tutup buku (aturan sudah ada di [03-inventory-foundation.md](03-inventory-foundation.md) poin 6, berlaku juga di sini meski implementasinya beda modul).

**Perpetual** (default, perilaku sebelum poin ini) — setiap mutasi (Penerimaan Barang, Pengiriman/Penjualan langsung, Retur Vendor/Customer, Pemakaian Sendiri, Stok Opname) langsung posting jurnal HPP/Persediaan saat itu juga, pakai `costing_method` (FIFO layer atau Average) sesuai poin F.

**Periodik** — HPP dihitung **1x di akhir periode** lewat Stok Opname (bukan per-transaksi), pakai formula klasik: **Persediaan Awal + Pembelian − Persediaan Akhir = HPP**. Keputusan desain (2026-08-30):
- **Qty tetap real-time** — `qty_on_hand` tetap ke-update tiap transaksi (Penerimaan/Pengiriman/Retur/Pemakaian), jadi validasi "stok tidak cukup" & kolom qty di Kartu Stok tetap jalan normal. Yang ditunda cuma **posting nilai/jurnal**, bukan qty.
- **Tidak ada FIFO layer/avg_cost/audit log** (`c18.stock.layer`/`.consumption`/`.movement`) yang dibuat selama periode berjalan — mesin costing FIFO/Average (poin F) sepenuhnya nonaktif dalam mode ini. Konsekuensi: **Kartu Stok kolom nilai kosong/tidak berlaku** selama mode Periodik aktif (cuma kolom qty yang valid) — beda dari Perpetual yang selalu lengkap qty+nilai.
- **Akun "Pembelian" baru** (`5-1100`, tipe HPP) — Penerimaan Barang dalam mode Periodik debit ke akun ini, **bukan** ke Persediaan (`1-1300`) seperti Perpetual. Kredit Hutang tetap sama (GRNI/Hutang Usaha, tidak berubah).
- **Akumulator per produk** — 2 field baru di `c18.product`: `periodic_purchases_qty`/`periodic_purchases_value` (total qty & nilai yang sudah masuk akun Pembelian sejak periode berjalan/opname terakhir, di-nolkan tiap kali Stok Opname periodik diposting) dan `periodic_book_value` (nilai Persediaan produk ini per penutupan periodik terakhir — jadi "Persediaan Awal" utk formula berikutnya).
- **Transaksi sisi keluar (Pengiriman, Penjualan langsung tanpa Pengiriman, Pemakaian Sendiri) TIDAK posting jurnal HPP/Persediaan sama sekali** selama Periodik aktif — cuma qty yang berkurang, tanpa leg akuntansi (kalau nilai jadi 0, dokumen tetap `posted` tapi `move_id` kosong, bukan bikin jurnal nilai 0). Konsisten dgn semangat "murni administratif sampai penutupan", dan selisih akibat pemakaian/penjualan otomatis "kefolding" ke formula HPP saat opname (persediaan akhir jadi lebih kecil krn barang sudah keluar, tanpa perlu dicatat terpisah).
- **Retur Barang Customer** (barang balik masuk) — simetris dgn sisi keluar di atas: qty nambah balik, **tanpa** masuk akumulator Pembelian (bukan pembelian baru) dan tanpa jurnal Persediaan/HPP (krn sisi keluarnya juga tidak pernah posting nilai).
- **Retur Barang Vendor** (barang keluar balik ke vendor) — beda dari retur customer krn ini justru **mengurangi** akumulator Pembelian (reverse penerimaan yang sudah menambah `periodic_purchases_*`), pakai rata-rata `periodic_purchases_value / periodic_purchases_qty` sbg harga reversal (bukan FIFO-terbaru krn tidak ada layer di mode ini). Jurnalnya debit Hutang / **kredit Pembelian** (bukan kredit Persediaan seperti Perpetual).
- **Stok Opname jadi titik penutupan periodik** — field baru `unit_cost` (input manual, default disarankan dari `_last_cost()` tapi editable krn sifatnya estimasi akhir periode) dipakai buat menilai Persediaan Akhir (`qty_physical × unit_cost`). Posting-nya bukan lagi "selisih × harga" (itu logic Perpetual), tapi jurnal penutupan 3-baris: (1) kredit penuh **Pembelian** (nolkan akumulator), (2) sesuaikan **Persediaan** dari `periodic_book_value` (awal) ke nilai akhir baru, (3) **HPP** sbg angka penyeimbang = Persediaan Awal + Pembelian − Persediaan Akhir. Setelah posting: `periodic_book_value` diupdate ke nilai akhir baru, `periodic_purchases_qty`/`_value` dinolkan, `qty_on_hand` disamakan ke `qty_physical` (sama seperti Perpetual).

**Akun "Pembelian" auto archive/unarchive** (2026-08-30, susulan) — akun `5-1100` selalu ada di CoA (data master statis, tidak bisa dibuat kondisional saat load), tapi biar tidak membingungkan muncul di daftar akun saat Perpetual aktif (padahal tidak pernah kepakai), `res.company.write()` di-override: tiap kali `inventory_system` diubah, akun ini otomatis di-archive (`active=False`) kalau Perpetual, di-unarchive (`active=True`) kalau Periodik. Default awal (fresh install) `active=False` di data XML, konsisten dgn default `inventory_system='perpetual'`. `env.ref()`/posting jurnal tetap jalan normal ke akun yang di-archive (archive cuma soal visibility di list/dropdown, bukan menghalangi record dipakai).

**Kenapa desain ini (bukan periodik murni tanpa qty tracking sama sekali)**: dipertimbangkan opsi "murni periodik" (qty juga cuma diketahui dari Stok Opname, sesuai teori akuntansi klasik), tapi ditolak krn bisnis distributor (skenario testing PT Roda Sejahtera) butuh tahu estimasi stok tersedia buat validasi Pengiriman & referensi harian — kalau qty juga blank di antara 2 opname, fitur "cek stok cukup/tidak" tidak bisa jalan sama sekali. Trade-off yang diambil: qty selalu akurat real-time (operasional), nilai/HPP resmi cuma final saat opname (akuntansi).
